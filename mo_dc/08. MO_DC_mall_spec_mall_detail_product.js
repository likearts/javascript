...
(function(window, angular, undefined) {
    ...
    function($http,LotteCommon,commInitData){
        return function(){
            return $http({
                url:LotteCommon.specMallDetailProductsData,
                params:{
    ...
    app.factory( 'dataFilter', [function(){
        return function(data){
            ...
            if( !data.relatedPrdList || !data.relatedPrdList.items ) data.relatedPrdList={items:[]};
            return data;
        }
    }])
     ...
     * - 핀터레스트 상세 컨트롤러
     */
    app.controller('specMallDetailProductCtrl', ['$scope', 'LotteCommon','commInitData','$timeout','api','dataFilter',
    function($scope, LotteCommon, commInitData, $timeout, api, dataFilter) {
        ...
        function loadCompleted(res){
            ...
            // 공유 이미지
            $scope.share_img = $scope.bindData.relatedPrdList.items[0].imgUrl;
        ...
        $scope.goProduct = function(){
            ...
            URL+="&disp_no="+$scope.kShopUI.dispNo+"&ptrNo="+$scope.kShopUI.dispNo;
            location.href=URL;
        }
    ...
    app.directive('lotteContainer', [function() {
        return {
            templateUrl : '/lotte/resources_dev/mall/spec_mall_detail_product_container.html',
            replace : true,
            link : function(scope) {
