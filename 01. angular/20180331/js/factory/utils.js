    ...
    .factory('scrollPage',function(baseService,baseObserver,$q,scrollOption){
        return function(){
            var defer = $q.defer(), per = 100, nextPoint = per-scrollOption.endPer;
                scrollTop = baseService.getObserver(baseObserver.scrollHeight).get();
            baseService.getObserver(baseObserver.scrollHeight).getDefer().then(null,null,function(updateHeight){
